// https://github.com/blikblum/pdfmake-utils/blob/v0.3.0/src/assetsloader.js
...
export class PdfAssetsLoader {
  ...
  load() {
    ...
    this.fontDefs.forEach(fontDef => {
      ...
      const vfsPath = isStandard
        ? `data/${fontDef.fileName}.afm`
        : fontDef.fileName;
      ...
      const fontFetch = fetchFile(fontURL, isStandard).then(data => {
        const fontInfo =
          this.fonts[fontDef.name] || (this.fonts[fontDef.name] = {});
